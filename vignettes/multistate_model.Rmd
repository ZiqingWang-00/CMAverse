...
Valeri et al. (2023) propose a novel methodology that leverages a
multistate joint modeling approach to quantify the effect of stochastic
interventions for a nonterminal intermediate time-to-event on a terminal
time-to-event outcome. The approach is here illustrated in a health
disparities context where interest lies in understanding the impact of a
stochastic intervention on a mediator in reducing health disparities and
in improving survival outcomes in a particular subgroup of the
population.

The following causal estimands have been introduced. The Residual
Disparity (RD) is defined as the difference in the probability of
surviving beyond time point s between the exposed group (A=1) and the
unexposed group (A=0), had the mediator distribution g been the fixed to
...
Consequently, the Total Effect (TE) is defined as the difference in the
...
Identification assumptions for these causal estimands is discussed in
Valeri et al. (2023).
...
An example is given to demonstrate how to use `cmest` when there is a
...
In this example, we use the `cmest` function to calculate RD and SD for
...
# run cmest()
sc_data_result100_10 = cmest(data = sc_data, 
                                  model = 'multistate', 
...
theo_mat # matrix that contains theoretical RD and SD for the specified time points
```
...
The plot shows that, on the time points specified, the RD, SD, and TE
estimated by the multistate `cmest` command have values similar to their
corresponding theoretical values. We here provide an interpretation of
the results for this simulated dataset:
