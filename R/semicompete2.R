...
s_point_est <- function(i, s_grid, data, newd000, newd010, mstate_form,
                           exposure, mediator, outcome, basec, mediator_event, event, 
                           trans, covs_df, formula_terms, method){
  print(i)
  # i is the boot_ind; extract the ith bootstrap sample
  boot_samps = boot_ind_df$boot_ind[[i]]
  dat = data[boot_samps,] 
  # make mstate data from the bootstrap sample
  mstate_data = make_mstate_dat(dat=dat, mediator, outcome, mediator_event, event, trans, covs_df)
  # create the multistate joint model object
  joint_mod = coxph(mstate_form, data = mstate_data, method = method)
...
system.time({
  test2 = s_point_est(i=10, s_grid, data, newd000=fixed_newd[[1]], newd010=fixed_newd[[2]], mstate_form,
                      exposure, mediator, outcome, basec, mediator_event, event,
                      trans, covs_df, formula_terms, method)
  test2
})
...
system.time({
  RD_vec_list <- sapply(i_grid, function(row) {
    print(row)
    s_point_est(i=row, s_grid, data, newd000=fixed_newd[[1]], newd010=fixed_newd[[2]], mstate_form,
                exposure, mediator, outcome, basec, mediator_event, event,
                trans, covs_df, formula_terms, method)
  })
})
...
boot_mean = apply(RD_vec_list, 1, mean)
boot_conf95_lower = apply(RD_vec_list, 1, function(x) quantile(x, probs = 0.025, na.rm = TRUE))
boot_conf95_upper = apply(RD_vec_list, 1, function(x) quantile(x, probs = 0.975, na.rm = TRUE))
